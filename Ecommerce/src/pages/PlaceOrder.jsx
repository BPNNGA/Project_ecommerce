        ...
        for (const item in cartItems[items]) {
          if (cartItems[items][item] > 0) {
            ...
            if (itemInfo) {
              ...
              if (typeof itemInfo.stock === 'number' && itemInfo.quantity > itemInfo.stock) {
                toast.error(`Insufficient stock for ${itemInfo.name}`);
                return;
              }
    ...
    <form onSubmit={onSubmitHandler} className="flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t">
      ...
      <div className="flex flex-col gap-4 w-full sm:max-w-[480px]">
        <div className="text-xl sm:text-2xl my-3 ">
          <Title text1={"DETAILS"} text2={"FOR DELIVERY"} />
        </div>
      ...
      <div className="mt-8">
        ...
        <div className="mt-12">
          ...
          <div className="w-full text-end mt-8">
            <button type="submit"  className="bg-black text-white text-xs px-10 py-4 cursor-pointer">PLACE ORDER</button>
          </div>
        </div>
      </div>
    </form>
